...
Les données utilisées dans ce Workshop sont disponibles dans la librairie `OmicCircos` de `R`. Elles reposent sur les bases de données fournies par ***The Cancer Genome Atlas Program*** (TCGA) contenant des données de patients atteints de différents sous-types de cancer du sein (Normal, LumA, LumB, Basal ou Her2) présentés dans le [lexique](#lexique). Voici la liste des tableaux utilisés :
