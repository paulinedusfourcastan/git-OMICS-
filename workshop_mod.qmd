...
Ce workshop [MOD](https://montpellier-omics-days.fr/) a été effectué par des étudiants du [master Statistiques et Sciences des Données](https://formations.umontpellier.fr/fr/formations/master-XB/master-mathematiques-ME157/statistique-et-science-des-donnees-ssd-KKIDCY8N.html) :
...
Les données utilisées dans ce Workshop sont disponibles dans la librairie `OmicCircos` de `R`. Elles reposent sur les bases de données fournies par ***The Cancer Genome Atlas Program*** (TCGA) contenant des données de patients atteints de différents sous-types de cancer du sein (Normal, LumA, LumB, Basal ou Her2) présentés présentés dans le [lexique](#lexique). Voici la liste des tableaux utilisés :
...
Pour pouvoir utiliser `ÒmicCircos`, il faut tout d'abord l'avoir installé à partir de [Bioconductor](https://bioconductor-org.translate.goog/packages/release/bioc/html/OmicCircos.html?_x_tr_sl=en&_x_tr_tl=fr&_x_tr_hl=fr&_x_tr_pto=tc/) puis l'appeler à l'aide de la fonction `library()`.
...
**En appliquant les compétences acquises jusqu'ici, remplissez les trous du code ci-dessous (indiqués par 'XXX') afin de représenter la densité et l'histogramme d'expression de l'ARNmessager des gènes pour le type de cancer *Her2* pour chacun des chromosomes.**
...
density <- data %>%
  ...
  xlab("Expression d'ARNm") +
  ylab("Fréquence") +
  labs(title="Densités d'expression génique pour le type de cancer Her2 sur chacun des chromosomes")
...
Pour la plupart des chromosomes, la densité observée est bimodale. Quelques chromosomes possèdent une densité différente. En effet, le chromosome $1$ possède un grand mode sur l'histogramme autour de la valeur de test $0$. Le chromosome $17$ est aussi atypique puisque la densité associée est assez étalée dû aux $4$ modes de valeurs assez dispersées présents dans l'histogramme.

::: {.callout-warning appearance="default" icon=false}
# POUR ALLER PLUS LOIN...

**De la même façon que le graphe précédent, remplissez les trous du code ci-dessous (indiqués par 'XXX') pour représenter graphiquement la densité et les histogrammes d'expression de l'ARNm des gènes pour chaque type de cancer sur chacun des chromosomes.**
...
density <- data %>%
  ...
  xlab("Expression d'ARNm") +
  ylab("Fréquence") +
  labs(title="Densités d'expression génique pour chaque type de cancer sur chacun des chromosomes",
       fill="Type de cancer")
...
Ce dernier graphe représente l'expression génique de plusieurs gènes pour chacun des types de cancer. Pour une meilleure lisibilité, seuls les $10$ premiers gènes de la base de données ont été utilisés mais il est possible d'en afficher plus. Par exemple, pour en afficher $15$, vous pourriez utiliser `data <- data[1:15,]`.
...
# création de l'objet
ggplot(data_long, aes(x=Gene, y=Expression, fill=Subtype)) +
  # définition du type de graphe
  ...
  theme(strip.text.x=element_text(size=8),
        plot.title=element_text(size=10, hjust=0.5, face="bold")) +
  # titres
  labs(title="Expression génique des gènes pour les différents types de cancer du sein",
       x="Gène",
